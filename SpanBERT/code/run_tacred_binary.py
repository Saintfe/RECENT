...
from pytorch_pretrained_bert.modeling_binary import BertForSequenceClassification
...
def evaluate(model, device, eval_dataloader, eval_label_ids, num_labels, verbose=True):
    ...
    for input_ids, input_mask, segment_ids, label_ids in eval_dataloader:
        ...
        labels_1 = label_ids.eq(0).eq(0).float().half()
        tmp_eval_loss = loss_fct(logits.view(-1), labels_1.view(-1))
    ...
    result = compute_f1(preds, eval_label_ids.eq(0).eq(0).tolist())
    result['accuracy'] = simple_accuracy(preds, eval_label_ids.eq(0).eq(0).tolist())
                ...
                # for step, batch in enumerate(train_batches):
                    ...
                    if (step + 1) % eval_step == 0:
                        ...
                        if args.do_eval:
                            ...
                            for pred, label in zip(preds[:20], eval_label_ids.eq(0).eq(0).tolist()[:20]):
