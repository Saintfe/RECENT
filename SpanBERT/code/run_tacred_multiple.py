...
class DataProcessor(object):
    ...
    def get_train_examples(self, data_dir, type_pair_id):
        """See base class."""
        return self._create_examples(
            self._read_json(os.path.join(data_dir, '%s_%s' % (ID_TO_TYPE_PAIR[type_pair_id], "train.json"))), "train")

    def get_dev_examples(self, data_dir, type_pair_id):
        """See base class."""
        return self._create_examples(
            self._read_json(os.path.join(data_dir, '%s_%s' % (ID_TO_TYPE_PAIR[type_pair_id], "dev.json"))), "dev")

    def get_test_examples(self, data_dir, type_pair_id):
        """See base class."""
        return self._create_examples(
            self._read_json(os.path.join(data_dir, '%s_%s' % (ID_TO_TYPE_PAIR[type_pair_id], "test.json"))), "test")
...
def main(args):
    ...
    label2id = LABEL_TO_ID[args.type_pair_id]
    ...
    special_tokens = {}
    if args.do_eval:
        eval_examples = processor.get_dev_examples(args.data_dir, args.type_pair_id)
    ...
    if args.do_train:
        train_examples = processor.get_train_examples(args.data_dir, args.type_pair_id)
    ...
    if args.do_eval:
        if args.eval_test:
            eval_examples = processor.get_test_examples(args.data_dir, args.type_pair_id)
            eval_features = convert_examples_to_features(
                eval_examples, label2id, args.max_seq_length, tokenizer, special_tokens, args.feature_mode)
            logger.info("***** Test *****")
            logger.info("  Num examples = %d", len(eval_examples))
